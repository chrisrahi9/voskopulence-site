...
// start on touch/pointer
const handlePointerDown: React.PointerEventHandler<HTMLButtonElement> = (e) => {
  // desktop keeps hover behavior; touch gets this flow
...
const scrollDown = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (Hls?.isSupported?.()) {
          const hls = new Hls({
            ...
            startLevel: -1,                 // auto
            maxBufferLength: 12,            // short buffer to reduce stalls
    ...
    const io = new IntersectionObserver(
      ([e]) => {
        // keep playing while even a sliver is visible
...
<header className="fixed top-0 z-50 w-full text-white/95">
  ...
  {/* --- CONTAINER (keep header content inside this) --- */}
...
{/* ===== Mobile curtain menu (FULL SCREEN, frosted, OUTSIDE header) ===== */}
<div
  id="mobile-menu" 
...
>
  {/* Frosted backdrop that covers EVERYTHING (header + video) */}
  <div
  ...
  <div className="relative z-10 flex flex-col h-full text-white">
    ...
    <nav className="flex-1 flex flex-col items-center justify-center gap-6 text-xl">
      ...
      {/* ===================== HERO ===================== */}
      <section className="relative z-0 w-screen overflow-visible">
...
<div className="absolute inset-x-0 top-0 -bottom-[16px]">
  ...
  className="
    ...
    group-data-[long=true]:scale-[1.06]         /* long-press grow */
  "
>
  {/* Soft glowing dot */}
  <div
    className="
      ...
      group-data-[show-arrow=true]:opacity-0     /* hide dot on touch */
      group-data-[long=true]:scale-110           /* tiny dot enlarge on long */
    "
  />
  ...
  <svg
    ...
    className="
      ...
      group-data-[show-arrow=true]:opacity-100
      group-data-[long=true]:translate-y-[2px]   /* match desktop hover drop */
    "
  >
    <path
...
>
  ...
  {/* Soft seam fade for smooth transition from hero */}
  ...
  {/* Background tint (subtle green wash) */}
...
<div className="pointer-events-none absolute -bottom-[2px] left-0 right-0 h-[4px] bg-white" />
  ...
  className="
    ...
    -mt-7 sm:-mt-9 lg:-mt-11   /* was -mt-8/-10/-14 (too high, got hidden) */
    rounded-t-[32px] sm:rounded-t-[40px]
    shadow-[0_-12px_30px_-12px_rgba(140,154,145,0.28)]
  ...
  {/* Soft seam fade for elegant transition */}
  <div className="absolute -top-6 left-0 w-full h-6 pointer-events-none bg-gradient-to-b from-black/20 to-transparent opacity-40" />
